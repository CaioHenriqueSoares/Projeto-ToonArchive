...
public ResponseEntity<List<Capitulo>> listarCapitulosDoManga(@PathVariable Long mangaId) {

    // Busca todos os capítulos desse mangá
    List<Capitulo> capitulos = capituloRepository.findByMangaId(mangaId);

    // Se quiser garantir a ordem por numero (recomendado):
    capitulos.sort((a, b) -> Double.compare(a.getNumero(), b.getNumero()));
